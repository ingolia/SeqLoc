...
module Bio.SeqLoc.ContigLocation ( -- * Sequence locations
                                   ContigLoc(..), fromStartEnd, fromPosLen
                                            
                                 -- * Locations and positions
                                 , bounds, startPos, endPos, posInto, posOutof
...
data ContigLoc = ContigLoc { offset5 :: !Pos.Offset   -- ^ The offset of the 5\' end of the location, as a 0-based index
                           , length :: !Pos.Offset    -- ^ The length of the location
...
bounds :: ContigLoc -> (Pos.Offset, Pos.Offset)
...
-- an error results.
seqData :: (SeqData.SeqData s, Stranded s) => s -> ContigLoc -> Maybe s
seqData sequ (ContigLoc seq5 len str)
    = liftM (stranded str) . (SeqData.subseq seq5 len) $ sequ
...
seqDataPad :: (SeqData.SeqData s, Stranded s) => s -> ContigLoc -> s
seqDataPad sequ (ContigLoc seq5 len str)
    = (stranded str) . (SeqData.subseqPad seq5 len) $ sequ
...
-- of the location.
extend :: (Pos.Offset, Pos.Offset) -> ContigLoc -> ContigLoc
...
-- spans (100, 150) and the sublocation is (110, 120) then 'clocInto'
-- will be (10, 20).
clocInto :: ContigLoc -> ContigLoc -> Maybe ContigLoc
clocInto subcloc tocloc
...
-- | Returns a sequence location produced by finding the inverse image
-- of a sublocation, with coordinates given relative to an enclosing
-- location, in the coordinate system of the enclosing location.  For
-- example, if the enclosing location spans (100, 150) and the
-- sublocation is (10, 20) then 'clocOutof' will be (110, 120).
clocOutof :: ContigLoc -> ContigLoc -> Maybe ContigLoc
clocOutof subcloc fromcloc
